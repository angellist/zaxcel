...
RSpec.describe Zaxcel::Functions::Max do
  let(:sheet) do
  ...
  it 'works with a range' do
    ...
    expect(described_class.new([range]).format(on_sheet: 'sheet')).to eq('MAX(A1:B1)')
  end

  it 'works with numerics' do
    expect(described_class.new([1, 2.4]).format(on_sheet: 'sheet')).to eq('MAX(1,2.4)')
  end
  ...
  it 'works with cell references' do
    ...
    expect(described_class.new([ref_1, ref_2, unresolved_ref]).format(on_sheet: 'sheet')).to eq('MAX(A1,B1)')
  end
end
