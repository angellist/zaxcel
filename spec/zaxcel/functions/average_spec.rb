...
RSpec.describe Zaxcel::Functions::Average do
  let(:sheet) do
    ...
    sheet.add_row!(:row).add!(:column, value: 2)
  ...
  it 'works with a range' do
    ...
    expect(described_class.new(range).format(on_sheet: 'sheet')).to eq('AVERAGE(A1:B1)')
  end
end
